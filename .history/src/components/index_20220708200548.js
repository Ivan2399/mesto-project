...
const popupImage = new PopupWithImage(config.popup.popupImage);
      ...
      render: () => {
        const card = new Card({
          ...
          likeCard: (cardID, likeElement, likeCounter) => {
            if (
              !likeElement.cardList.contains(config.card.cardElementLikeActive)
            ) {
              api
                .addCardLikes(cardID)
                .then((card) => {
                  likeElement.classList.add(config.card.cardElementLikeActive);
              ...
              api.deleteCardLikes(cardID).then((card) => {
                likeElement.classList.remove(config.card.cardElementLikeActive);
                if (card.likes.length > 0) {
                  likeCounter.textContent = card.likes.length;
                } else {
                  likeCounter.textContent = "";
